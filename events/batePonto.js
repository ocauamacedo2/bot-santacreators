...
export default async function setupBatePonto(client) {
  ...
  COLORS: { primary: 0x8A2BE2, ok: 0x22C55E, warn: 0xF59E0B, err: 0xEF4444 }
      ...
      // ===== PERSISTÊNCIA ROBUSTA EM DISCO (fonte oficial para Dash/Ranking) =====
    const BP_STATE_DIR = path.resolve(process.cwd(), "data", "sc_bp_monthly");

    function ensureBpStateDir() {
      try {
        fs.mkdirSync(BP_STATE_DIR, { recursive: true });
      } catch {}
    }

    function bpMonthFile(monthKey) {
      return path.join(BP_STATE_DIR, `${monthKey}.json`);
    }

    function blankMonth(monthKey) {
      return { monthKey, days: {} };
    }

    function loadMonthFromDisk(monthKey) {
      try {
        const file = bpMonthFile(monthKey);
        if (!fs.existsSync(file)) return blankMonth(monthKey);

        const raw = JSON.parse(fs.readFileSync(file, "utf8"));
        if (!raw || raw.monthKey !== monthKey || typeof raw.days !== "object") {
          return blankMonth(monthKey);
        }
    ...
    function mergeMonthStates(base, extra) {
      const out = blankMonth(base?.monthKey || extra?.monthKey || "unknown");

      for (const src of [base, extra]) {
        if (!src || typeof src.days !== "object") continue;

        for (const [dayKey, arr] of Object.entries(src.days || {})) {
          out.days[dayKey] ??= [];

          for (const entry of Array.isArray(arr) ? arr : []) {
            const exists = out.days[dayKey].some(
              (x) => `${x.uid}|${x.time}` === `${entry.uid}|${entry.time}`
            );
            if (!exists) out.days[dayKey].push(entry);
          }
        }
      }
    ...
    function saveMonthToDisk(state) {
      try {
        if (!state?.monthKey) return;
        ensureBpStateDir();

        const file = bpMonthFile(state.monthKey);
        const tmp = `${file}.tmp`;

        fs.writeFileSync(tmp, JSON.stringify(state, null, 2), "utf8");
        fs.renameSync(tmp, file);
      } catch (e) {
        console.error("[SC_BP] saveMonthToDisk error:", e);
      }
    }
...
const withinWindow = () => {
  ...
  return (CFG.WINDOWS || []).some((w) => hh >= w.start && hh < w.end);
    ...
    let SYNCING = false;
        ...
        const emptyMonth = (mk) => mergeMonthStates(blankMonth(mk), loadMonthFromDisk(mk));

    const ensureMonth = (mk) => {
      if (!STATE || STATE.monthKey !== mk) {
        STATE = emptyMonth(mk);
      }
    };
  ...
  return (STATE.days[dayKey] || []).some((e) => e.uid === userId);
      ...
      const diskState = loadMonthFromDisk(monthKey);

      if (!msg) {
        STATE = mergeMonthStates(emptyMonth(monthKey), diskState);
        saveMonthToDisk(STATE);
        ...
        try {
          ...
          const memState =
            STATE && STATE.monthKey === monthKey ? STATE : blankMonth(monthKey);

          STATE = mergeMonthStates(
            mergeMonthStates(diskState, remoteState),
            memState
          );
      ...
      try {
        ...
        const merged =
          parsed?.monthKey === monthKey
            ? mergeMonthStates(diskState, parsed)
            : diskState;

        saveMonthToDisk(merged);
        return merged;
      } catch {
        return loadMonthFromDisk(monthKey);
      }
    };
