...
export async function payEvtDashHandleInteraction(interaction, client) {
  ...
  if (interaction.isButton() && interaction.customId === "PEV_REMOVE_POINTS") {
    const hasPerm = ALLOWED_MANAGE_IDS.includes(interaction.user.id) || 
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("amount").setLabel("Quantidade a Remover").setStyle(TextInputStyle.Short).setRequired(true)
      )
    );
  ...
  if (interaction.isModalSubmit() && interaction.customId === "PEV_REMOVE_MODAL") {
    ...
    await interaction.reply({ content: `✅ Removidos **${amount}** pontos de <@${userId}> na semana atual (Pagamentos).`, ephemeral: true });
    return true;
  }
