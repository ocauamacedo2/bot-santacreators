...
function readBPStatesFromDisk(monthKeys = []) {
  ...
  try {
    ...
    console.error("[SC_GERAL_DASH] readBPStatesFromDisk error:", e);
  }
...
async function collectAllGeneral(client, mode = "light") {
  ...
  await scanChannelEmbeds(client, {
    ...
    onMessage: async (m) => {
      ...
      if (seenMessageIds.has(m.id)) { auditor.reject('poderes', 'duplicate_message'); return; } // Added duplicate check
      seenMessageIds.add(m.id);
      const emb = m.embeds?.[0];
      if (!emb) { auditor.reject('poderes', 'no_embed'); return; }
      if (!isPoderesRecordEmbed(emb)) { auditor.reject('poderes', 'invalid_embed'); return; }
...
for (const channelId of DOACAO_LOGS_CHANNEL_IDS) {
  await scanChannelEmbeds(client, {
    ...
    maxPages: 150,
    onMessage: async (m) => {
      auditor.addStats('doacoes', 'scanned');
      ...
      auditor.addStats('doacoes', 'uidOk');
      seenMessageIds.add(seenKey);
...
for (const channelId of CRONOGRAMA_LOGS_CHANNEL_IDS) {
  await scanChannelEmbeds(client, {
    ...
    onMessage: async (m) => {
      ...
      if (seenMessageIds.has(seenKey)) { auditor.reject('cronograma', 'duplicate_message'); return; }
      seenMessageIds.add(seenKey);
      const emb = m.embeds?.[0];
      if (!emb) { auditor.reject('cronograma', 'no_embed'); return; }
      if (!GERAL_PARSERS.isCronogramaApproved(emb)) { auditor.reject('cronograma', 'not_approved'); return; }
...
async function backfillGestaoThisWeek(client) {
  try {
    ...
    // ✅ conta primeiro em memória temporária
    let rmAprovados = 0;
    let rmReprovados = 0;
    let alinhamentos = 0;
    let eventosPoderes = 0;
    let poderesUtilizados = 0;
    let pagCriados = 0;
    let pagSolicitados = 0;
    let pagPagos = 0;
    let pagReprovados = 0;
  ...
  await scanChannelEmbeds(client, {
    ...
    if (CH_PAGAMENTOS_ID) {
      await scanCurrentWeekEmbeds(
        client,
        CH_PAGAMENTOS_ID,
        (emb) => isPaymentRecordEmbed(emb),
        async (_m, emb) => {
          pagCriados += 1;
          const status = pagamento_getStatus(emb);
          if (status.isSolicitado) pagSolicitados += 1;
          if (status.isPago) pagPagos += 1;
          if (status.isReprovado) pagReprovados += 1;
        },
        25
      );
    }
    ...
    // ✅ só grava no final
    st.weekly.rmAprovados[wkNow] = rmAprovados;
    st.weekly.rmReprovados[wkNow] = rmReprovados;
    st.weekly.alinhamentos[wkNow] = alinhamentos;
    st.weekly.eventosPoderes[wkNow] = eventosPoderes;
    st.weekly.poderesUtilizados[wkNow] = poderesUtilizados;
    st.weekly.pagCriados[wkNow] = pagCriados;
    st.weekly.pagSolicitados[wkNow] = pagSolicitados;
    st.weekly.pagPagos[wkNow] = pagPagos;
    st.weekly.pagReprovados[wkNow] = pagReprovados;
...
async function backfillVipAndOthersThisWeek(client) {
  try {
    ...
    // ✅ temporários
    let vipCriados = 0;
    let vipSolicitados = 0;
    let vipPagos = 0;
    let vipReprovados = 0;
    let halldafama = 0;
    let eventosdiarios = 0;
    ...
    await scanCurrentWeekEmbeds(
      ...
      async (_m, emb) => {
        ...
        if (status.isSolicitado) vipSolicitados += 1;
        if (status.isPago) vipPagos += 1;
        if (status.isReprovado) vipReprovados += 1;
      },
      40
    );
    ...
    // ✅ grava só no final
    st.weekly.vipCriados[wkNow] = vipCriados;
    st.weekly.vipSolicitados[wkNow] = vipSolicitados;
    st.weekly.vipPagos[wkNow] = vipPagos;
    st.weekly.vipReprovados[wkNow] = vipReprovados;
    st.weekly.halldafama[wkNow] = halldafama;
    st.weekly.eventosdiarios[wkNow] = eventosdiarios;
...
async function backfillExtrasThisWeek(client) {
  try {
    ...
    if (DOACAO_LOGS_CHANNEL_ID) {
      await scanCurrentWeekEmbeds(
        ...
        async (_m, emb) => {
          if (doacaoWasScoredFromEmbed(emb)) doacoes += 1;
        },
        25
      );
    }
    ...
    if (CONVITES_LOGS_CHANNEL_ID) {
      await scanCurrentWeekEmbeds(
        ...
        (emb) => {
          const titleOk = isConviteLogEmbed(emb);
          const footer = String(emb?.footer?.text || emb?.data?.footer?.text || "");
          return titleOk || footer.includes("Líderes");
        },
        async () => {
          convites += 1;
        },
  ...
  await scanChannelEmbeds(client, {
    ...
    // ✅ grava só no final
    st.weekly.doacoes[wkNow] = doacoes;
    st.weekly.convites[wkNow] = convites;
    st.weekly.perguntas[wkNow] = perguntas;
    st.weekly.vendas[wkNow] = vendas;
    st.weekly.cronograma[wkNow] = cronograma;
    st.weekly.presencas[wkNow] = presencas;
    st.weekly.correcao[wkNow] = correcao;
            ...
            .setDescription(
              [
               ...
               `• Perguntas: **${mPerg}**`,
                ...
                `• Presenças Confirmadas: **${mPresencas}**`,
                `• Hall da Fama: **${mHall}**`,
                `• Eventos Diários: **${mEventosDiarios}**`,
                `• Correções de Entrevista: **${mCorrecao}**`,
